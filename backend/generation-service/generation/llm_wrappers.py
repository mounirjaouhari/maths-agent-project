...
import openai
from shared.exceptions import InternalServerError, LLMAPIError
...
class LLMWrapper:
    ...
    async def call_llm(self, model_name: str, prompt: str,
        ...
        try:
            if model_name.startswith("gpt"):
                ...
                raise BadRequestException(
                    detail=f"Modèle LLM non supporté: {model_name}")
        except Exception as e:
            logger.error(
                f"Erreur inattendue lors de l'appel LLM pour le modèle {model_name}: {e}",
                exc_info=True,
            )
            raise InternalServerError(
                detail=f"Erreur interne lors de l'appel au LLM {model_name}.")

    async def _call_openai(self, model_name: str, prompt: str,
                           params: Dict[str, Any]) -> str:
        """Appelle l'API OpenAI."""
        if not openai.api_key:
            raise LLMAPIError(llm_name="OpenAI",
                              original_detail="Clé API OpenAI non configurée.")
        ...
        try:
            ...
            openai_params = {
                ...
                params.get("max_tokens",
                           1024),  # Valeur par défaut raisonnable
                "temperature": params.get("temperature", 0.7),
                "top_p": params.get("top_p", 1.0),
                "frequency_penalty": params.get("frequency_penalty", 0.0),
                "presence_penalty": params.get("presence_penalty", 0.0),
                # Permettre des paramètres spécifiques
                **params.get("openai_specific_params", {}),
            }
            ...
            raise LLMAPIError(
                llm_name="OpenAI",
                original_detail=f"Erreur d'authentification: {e.user_message}",
            )
        except openai.error.RateLimitError as e:
            raise LLMAPIError(
                llm_name="OpenAI",
                original_detail=f"Limite de taux atteinte: {e.user_message}",
            )
        except openai.error.APIError as e:
            raise LLMAPIError(
                llm_name="OpenAI",
                original_detail=f"Erreur API OpenAI: {e.user_message}",
            )
        except Exception as e:
            logger.error(
                f"Erreur inattendue lors de l'appel OpenAI {model_name}: {e}",
                exc_info=True,
            )
            raise LLMAPIError(llm_name="OpenAI",
                              original_detail=f"Erreur inconnue: {e}")

    async def _call_gemini(self, model_name: str, prompt: str,
                           params: Dict[str, Any]) -> str:
        ...
        try:
            ...
            response = await model.generate_content_async(
                prompt, generation_config=gemini_params)
            ...
            if response.candidates and response.candidates[0].content.parts:
                ...
                if response.prompt_feedback and response.prompt_feedback.block_reason:
                    ...
                    raise LLMAPIError(
                        llm_name="Gemini",
                        original_detail=f"Contenu bloqué par le filtre de sécurité: {block_reason}",
                    )
                raise LLMAPIError(
                    llm_name="Gemini",
                    original_detail="Aucun contenu généré par le modèle.",
                )

        except Exception as e:
            logger.error(f"Erreur lors de l'appel Gemini {model_name}: {e}",
                         exc_info=True)
            raise LLMAPIError(llm_name="Gemini",
                              original_detail=f"Erreur inconnue: {e}")

    async def _call_anthropic(self, model_name: str, prompt: str,
                              params: Dict[str, Any]) -> str:
        """Appelle l'API Anthropic (Claude)."""
        if not self.anthropic_client:
            raise LLMAPIError(
                llm_name="Anthropic",
                original_detail="Client Anthropic non initialisé (clé API manquante).",
            )
        ...
        try:
            ...
            raise LLMAPIError(
                llm_name="Anthropic",
                original_detail=f"Erreur API Anthropic: {e.response.text}",
            )
        except Exception as e:
            logger.error(f"Erreur lors de l'appel Anthropic {model_name}: {e}",
                         exc_info=True)
            raise LLMAPIError(llm_name="Anthropic",
                              original_detail=f"Erreur inconnue: {e}")
