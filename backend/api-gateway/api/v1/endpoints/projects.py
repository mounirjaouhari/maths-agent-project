...
from ..auth.dependencies import (
    get_current_user_id,  # Pour obtenir l'ID de l'utilisateur authentifié
)
...
async def create_project(
        ...
        current_user_id: str = Depends(
            get_current_user_id),  # Dépendance pour l'authentification
):
    """
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour la création de projet: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de la création du projet.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de persistance: {e}")
        raise ServiceUnavailableException(
            detail="Le service de persistance n'est pas disponible.")
...
async def list_projects(
        ...
        status_filter: Optional[
            str] = None,  # Renommé pour éviter le conflit avec 'status'
):
    """
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour lister les projets: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de la récupération des projets.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de persistance: {e}")
        ...
        logger.error(f"Erreur inattendue lors de la liste des projets: {e}",
                     exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur interne du serveur.",
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            document_response = await client.get(
                # Endpoint à implémenter dans persistence-service
                f"{settings.PERSISTENCE_SERVICE_URL}/internal/documents/project/{project_id}"
            )
            ...
            content_blocks_response = await client.get(
                ...
                f"{settings.PERSISTENCE_SERVICE_URL}/internal/content-blocks/version/{current_version_id}"
            )
            ...
            project_detail = ProjectDetailResponse(
                **project_data,
                document_structure=document_version_data.get(
                    "content_structure", {}),
                content_blocks=[
                    ContentBlockResponse(**cb) for cb in content_blocks_data
                ],
            )
        ...
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour récupérer le projet: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de la récupération du projet.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de persistance: {e}")
        ...
        logger.error(
            f"Erreur inattendue lors de la récupération du projet: {e}",
            exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur interne du serveur.",
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.put(
                f"{settings.PERSISTENCE_SERVICE_URL}/internal/projects/{project_id}",
                json=project_update_data.model_dump(
                    exclude_unset=True
                ),  # N'envoie que les champs qui sont définis
            )
            response.raise_for_status()
        ...
        if e.response.status_code == status.HTTP_404_NOT_FOUND:
            raise NotFoundException(
                detail=f"Projet avec l'ID {project_id} non trouvé.")
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour la mise à jour de projet: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de la mise à jour du projet.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de persistance: {e}")
        ...
        logger.error(
            f"Erreur inattendue lors de la mise à jour du projet: {e}",
            exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur interne du serveur.",
    ...
    try:
        ...
        if e.response.status_code == status.HTTP_404_NOT_FOUND:
            raise NotFoundException(
                detail=f"Projet avec l'ID {project_id} non trouvé.")
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour la suppression de projet: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de la suppression du projet.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de persistance: {e}")
        ...
        logger.error(
            f"Erreur inattendue lors de la suppression du projet: {e}",
            exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur interne du serveur.",
...
@router.post(
    "/{project_id}/signal",
    response_model=ProjectResponse,
    summary="Envoyer un signal utilisateur au moteur de workflow",
)
async def send_workflow_signal(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if existing_project.get("user_id") != current_user_id:
                raise ForbiddenException(
                    detail="Vous n'êtes pas autorisé à envoyer un signal pour ce projet."
                )

            # Envoyer le signal au service de workflow
            response = await client.post(
                # Endpoint à implémenter dans workflow-service
                f"{settings.WORKFLOW_SERVICE_URL}/internal/projects/{project_id}/signal",
        ...
        if e.response.status_code == status.HTTP_404_NOT_FOUND:
            raise NotFoundException(
                detail=f"Projet avec l'ID {project_id} non trouvé.")
        logger.error(
            f"Erreur lors de l'appel au service de workflow pour le signal: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de workflow lors du traitement du signal.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de workflow: {e}")
        raise ServiceUnavailableException(
            detail="Le service de workflow n'est pas disponible.")
    except Exception as e:
        logger.error(f"Erreur inattendue lors de l'envoi du signal: {e}",
                     exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur interne du serveur.",
...
async def get_qc_report_for_block(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            project_response = await client.get(
                # document_id est en fait le project_id dans notre modèle simplifié
                f"{settings.PERSISTENCE_SERVICE_URL}/internal/projects/{version_data['document_id']}"
            )
            project_response.raise_for_status()
            project_data = project_response.json()
            ...
            if project_data.get("user_id") != current_user_id:
                raise ForbiddenException(
                    detail="Vous n'êtes pas autorisé à accéder à ce rapport QC."
                )
            ...
            if not qc_report:
                raise NotFoundException(
                    detail=f"Rapport QC non disponible pour le bloc {block_id}."
                )
        ...
        if e.response.status_code == status.HTTP_404_NOT_FOUND:
            raise NotFoundException(
                detail=f"Bloc de contenu avec l'ID {block_id} non trouvé.")
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour le rapport QC: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de la récupération du rapport QC.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de persistance: {e}")
        ...
        logger.error(
            f"Erreur inattendue lors de la récupération du rapport QC: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def download_exported_document(
    export_id:
    str,  # Dans notre modèle actuel, export_id pourrait être le document_id ou une tâche d'exportation
    current_user_id: str = Depends(get_current_user_id),
):
    """
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if export_status_data.get("status") != "completed":
                raise BadRequestException(
                    detail="L'exportation n'est pas encore terminée ou a échoué.")
            ...
            if not download_url:
                raise NotFoundException(
                    detail="URL de téléchargement non disponible pour cette exportation."
                )
        ...
        if e.response.status_code == status.HTTP_404_NOT_FOUND:
            raise NotFoundException(
                detail=f"Exportation avec l'ID {export_id} non trouvée.")
        logger.error(
            f"Erreur lors de l'appel au service d'exportation pour le statut: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service d'exportation lors de la récupération du statut.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service d'exportation: {e}")
        raise ServiceUnavailableException(
            detail="Le service d'assemblage/exportation n'est pas disponible.")
    except Exception as e:
        logger.error(
            f"Erreur inattendue lors du téléchargement du document exporté: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
