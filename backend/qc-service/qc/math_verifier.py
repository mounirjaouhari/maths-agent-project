...
from shared.models import (  # Importe les modèles pour les rapports QC
    QCProblem, QCReport,
)
...
class MathVerifier:
    ...
    def __init__(self, kb_service_url: str, external_tools_path: str):
        ...
        self.external_tools_path = (
            external_tools_path  # Chemin où les exécutables comme Z3 sont installés
        )
        self.sympy_client = (
            self._initialize_sympy()
        )  # SymPy est une bibliothèque Python, pas un client externe HTTP
    ...
    async def _call_z3_solver(self, smt_lib_formula: str) -> Dict[str, Any]:
        ...
        """
        z3_path = (
            # Assurez-vous que Z3 est exécutable ici
            f"{self.external_tools_path}/z3")
        try:
            ...
            stdout, stderr = await process.communicate(
                smt_lib_formula.encode())

            if process.returncode != 0:
                logger.error(
                    f"Z3 process exited with error code {process.returncode}: {stderr.decode()}"
                )
                raise ExternalToolError(
                    tool_name="Z3",
                    detail=f"Z3 execution failed: {stderr.decode()}")
            ...
            if "unsat" in output:
                ...
                return {
                    "status": "sat",
                    "model": output,
                }  # Le modèle peut être parsé plus finement
            else:
            ...
            raise ExternalToolError(
                tool_name="Z3",
                detail=f"Z3 executable not found at {z3_path}. Ensure it's installed and path is correct.",
            )
        except Exception as e:
            logger.error(f"Erreur lors de l'appel à Z3: {e}", exc_info=True)
            raise ExternalToolError(
                tool_name="Z3",
                detail=f"Erreur inattendue lors de l'appel à Z3: {e}")
    ...
    async def _call_wolfram_alpha_api(self, query: str) -> Dict[str, Any]:
        ...
        logger.warning(
            "Wolfram Alpha API call is a placeholder. Not implemented.")
    ...
    def _parse_latex_to_sympy(self, latex_expr: str) -> Any:
        ...
        try:
            ...
            expr_str = (latex_expr.replace("\\frac{", "(").replace(
                "}{", ")/(").replace("}", ")"))
            expr_str = expr_str.replace("\\sqrt{",
                                        "sqrt(").replace("\\cdot", "*")
            expr_str = expr_str.replace("\\sum",
                                        "Sum").replace("\\int", "Integral")
            ...
            logger.warning(
                f"SymPy parsing error for LaTeX '{latex_expr}': {e}")
            raise BadRequestException(
                detail=f"Erreur de parsing LaTeX pour SymPy: {e}")
        except Exception as e:
            logger.error(
                f"Erreur inattendue lors du parsing LaTeX vers SymPy: {e}",
                exc_info=True,
            )
            raise InternalServerError(
                detail=f"Erreur interne de parsing LaTeX: {e}")

    async def verify_mathematical_statement(
            self,
            statement_latex: str,
        ...
        problems = []
        confidence = (
            1.0  # Commence avec une confiance élevée, réduite en cas de problème
        )

        # 1. Vérification de syntaxe LaTeX de base (peut être plus approfondie)
        if not statement_latex.strip().startswith(
                "$") or not statement_latex.strip().endswith("$"):
            # Très basique, vérifie si c'est une formule inline
            problems.append(
                QCProblem(
                    type="formatting_error",
                    severity="minor",
                    description="La formule LaTeX ne semble pas correctement délimitée par des '$'.",
                ))
            confidence -= 0.1
        ...
        try:
            ...
            if not match:
                problems.append(
                    QCProblem(
                        type="math_error",
                        severity="critical",
                        description="Impossible d'extraire une expression mathématique valide du LaTeX.",
                    ))
                confidence = 0.0
                return {
                    "status": "error",
                    "details": "Parsing LaTeX échoué.",
            ...
            if "=" in expr_latex_clean:
                ...
                if self.sympy_client.simplify(lhs_sympy - rhs_sympy) == 0:
                    logger.info(
                        f"SymPy: Égalité '{expr_latex_clean}' vérifiée.")
                    ...
                    problems.append(
                        QCProblem(
                            type="math_error",
                            severity="critical",
                            description=f"L'égalité '{expr_latex_clean}' semble fausse selon SymPy.",
                        ))
                    confidence = 0.1
            elif (">=" in expr_latex_clean or "<=" in expr_latex_clean
                  or ">" in expr_latex_clean or "<" in expr_latex_clean):
                # Vérification d'inégalités (plus complexe avec SymPy seul, souvent mieux avec Z3)
                problems.append(
                    QCProblem(
                        type="math_error",
                        severity="warning",
                        description=f"Vérification d'inégalité '{expr_latex_clean}' nécessite un solveur plus avancé.",
                    ))
                confidence -= 0.2
            else:
                # Si c'est juste une expression, on peut essayer de la simplifier ou de l'évaluer
                simplified_expr = self.sympy_client.simplify(
                    self._parse_latex_to_sympy(expr_latex_clean))
                logger.info(
                    f"SymPy: Expression '{expr_latex_clean}' simplifiée en '{simplified_expr}'."
                )
            ...
            problems.append(
                QCProblem(
                    type="formatting_error",
                    severity="critical",
                    description=f"Erreur de parsing LaTeX pour SymPy: {e.detail}",
                ))
            confidence = 0.0
        except Exception as e:
            problems.append(
                QCProblem(
                    type="math_error",
                    severity="critical",
                    description=f"Erreur interne lors de la vérification SymPy: {e}",
                ))
            confidence = 0.0
                ...
                if z3_result.get("status") == "unsat":
                    problems.append(
                        QCProblem(
                            type="math_error",
                            severity="critical",
                            description=f"Assertion '{statement_latex}' réfutée par Z3.",
                        ))
                    confidence = 0.0
                    ...
                    problems.append(
                        QCProblem(
                            type="math_error",
                            severity="warning",
                            description=f"Assertion '{statement_latex}' est satisfiable mais pas nécessairement universellement vraie.",
                        ))
                    confidence -= 0.3
                else:
                    problems.append(
                        QCProblem(
                            type="math_error",
                            severity="minor",
                            description=f"Z3 n'a pas pu déterminer la validité de l'assertion '{statement_latex}'.",
                        ))
                    confidence -= 0.1
            except ExternalToolError as e:
                problems.append(
                    QCProblem(
                        type="external_tool_error",
                        severity="major",
                        description=f"Erreur Z3: {e.detail}",
                    ))
                confidence -= 0.2
            except Exception as e:
                problems.append(
                    QCProblem(
                        type="external_tool_error",
                        severity="major",
                        description=f"Erreur inattendue lors de l'appel Z3: {e}",
                    ))
                confidence -= 0.2
    ...
    async def analyze_proof_step(
        self,
        step_latex: str,
        previous_steps_context: str,
        theorem_references: List[UUID],
        ...
        problems = []
        confidence = (
            0.5  # Commence avec une confiance moyenne pour les preuves complexes
        )

        logger.warning(
            "L'analyse d'étape de preuve est une fonctionnalité avancée et n'est pas complètement implémentée."
        )
        problems.append(
            QCProblem(
                type="other",
                severity="warning",
                description="Analyse d'étape de preuve non entièrement implémentée, vérification limitée.",
            ))

        # Exemple: Vérifier si l'étape contient une assertion mathématique simple
        result_statement = await self.verify_mathematical_statement(
            step_latex, context)
        if result_statement["status"] in ["refuted", "error"]:
            problems.append(
                QCProblem(
                    type="math_error",
                    severity="critical",
                    description=f"Assertion dans l'étape de preuve est fausse: {result_statement.get('details')}",
                ))
            confidence = 0.0
            ...
            problems.append(
                QCProblem(
                    type="math_error",
                    severity="major",
                    description=f"Assertion dans l'étape de preuve a des problèmes: {result_statement.get('details')}",
                ))
            confidence -= 0.3
