...
from .security import oauth2_scheme, decode_access_token # Importe le schéma OAuth2 et la fonction de décodage
...
async def require_admin_role(current_user: dict = Depends(get_current_user)):
    ...
    if current_user.get("role") != "admin":
        raise ForbiddenException(detail="Accès requis pour les administrateurs.")
    return current_user
