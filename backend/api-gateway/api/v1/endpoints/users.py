...
from shared.models import UserBase, UserRegister, UserResponse
...
async def register_user(user_data: UserRegister):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            )  # Lève une exception pour les codes d'erreur HTTP (4xx, 5xx)
        ...
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour l'enregistrement: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de l'enregistrement.",
        )
        ...
        logger.error(
            f"Erreur inattendue lors de l'enregistrement de l'utilisateur: {e}",
            exc_info=True,
        )
        raise HTTPException(
...
async def login_for_access_token(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            user = UserBase(
                **user_data
            )  # Utiliser UserBase car le mot de passe haché n'est pas exposé ici
            user.password_hash = user_data.get(
                "password_hash"
            )  # Récupérer le hash du mot de passe pour la vérification
        ...
        if e.response.status_code == status.HTTP_404_NOT_FOUND:
            raise UnauthorizedException(
                detail="Nom d'utilisateur ou mot de passe incorrect.")
        logger.error(
            f"Erreur lors de l'appel au service de persistance pour la connexion: {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Erreur du service de persistance lors de la connexion.",
        )
    except httpx.RequestError as e:
        logger.error(
            f"Erreur réseau lors de l'appel au service de persistance: {e}")
        ...
        logger.error(
            f"Erreur inattendue lors de la connexion de l'utilisateur: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
